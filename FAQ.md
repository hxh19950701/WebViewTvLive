...
| 代号   | 名称 | 解释 | 解决方式 |
|--------|-----------------|-----------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|
| AW     | Android WebView | 设备厂商自编译的 WebView，很难更新 | 1. 版本过低，升级（如果厂商还提供更新）。<br/> 2. 更换成 Google WebView（比较困难，一般需要 Root 或反编译）。<br/> 3. 打开“全屏兼容模式”。<br/> 4. 使用 X5 内核。 |
| GW     | Google WebView  | Google 提供的 WebView| 1. 版本过低，升级（[可从这里获取](https://www.apkmirror.com/apk/google-inc/android-system-webview/)）。<br/> 2. 使用 X5 内核。            |
...
| UKN | 未知 WebView | 系统 WebView 工作不正常 | 1. 安装或更换成 Google WebView。<br/> 2. 使用 X5 内核。 |
